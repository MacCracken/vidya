...
func main() {
	...
	if !ok || dec != 1234567890 || dn != len(enc) {
		panic("roundtrip")
	}

	bomb := bytes.Repeat([]byte{0xFF}, 11)
	if _, _, ok := decodeVarint(bomb); ok {
		panic("overflow")
	}

	payload := []byte("hello, world")
	frame := encodeFrame(payload)
	if len(frame) != 13 || frame[0] != 12 {
		panic("frame")
	}
	dec2, c, ok := decodeFrame(frame, MaxMsgSize)
	if !ok || c != 13 || !bytes.Equal(dec2, payload) {
		panic("frame rt")
	}
	...
	if _, _, ok := decodeFrame(trunc, MaxMsgSize); ok {
		panic("trunc")
	}

	over := encodeVarint(9999)
	if _, _, ok := decodeFrame(over, MaxMsgSize); ok {
		panic("oversize")
	}
